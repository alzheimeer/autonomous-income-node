...
export interface ICopyMetricsRecorder {
  recordSignal(s: CopySignal | EnrichedSignal, v?: SignalValidationResult): Promise<void>;
  recordSignalBatch(b: Array<{ signal: CopySignal | EnrichedSignal; validation?: SignalValidationResult }>): Promise<void>;
  getSignalById(id: string): Promise<CopySignalRecord | null>;
  getSignalsByWallet(addr: string, lim?: number): Promise<CopySignalRecord[]>;
  getRecentSignals(lim: number): Promise<CopySignalRecord[]>;
`;

writeFileSync(join(process.cwd(), 'src/copy-trading/modules/CopyMetricsRecorder.ts'), content, 'utf8');
